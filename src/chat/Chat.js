...
function Chat(props) {
  ...
  const messages = props.chatkit.messages.map(message => ({
      id: message.id,
      isOwnMessage: message.sender.id === props.chatkit.currentUser.id,
      ...
      textContent: message.parts[0].payload.content
  }));
